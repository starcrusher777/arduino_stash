...
#define TFT_CS  -1 // TFT select pin
...
void setup(void) {
  ...
  tft.init(240, 240, SPI_MODE3);           // Init ST7789 172x320
  ...
  stat = reader.drawBMP("/frame0001.bmp", tft, 0, 0);
  ...
  stat = reader.bmpDimensions("/robot.bmp", &width, &height);
  reader.printStatus(stat);   // How'd we do?
  ...
  stat = reader.loadBMP("/frame0002.bmp", img);
  reader.printStatus(stat); // How'd we do?
...
void loop() {
  for(int r=0; r<4; r++) { // For each of 4 rotations...
    ...
    for(int i=0; i<4; i++) {
      reader.drawBMP("/frame0001.bmp", tft,
